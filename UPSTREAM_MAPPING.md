...
Target Upstream Tag: `github.com/charmbracelet/x` monorepo at module tag `ansi/v0.11.2`

This mapping accounts for **every** file of the upstream `charmbracelet/x` monorepo at the
`ansi/v0.11.2` tag. The ported module is `github.com/charmbracelet/x/ansi`, located in the
...
| `ansi/color.go` | `src/color.rs` | `BasicColor`, `IndexedColor`, `RGBColor`, `Convert256`, `Convert16`, `ReadStyleColor`, `ansiHex` palette |
| `ansi/util.go` | `src/util.rs` | `Strip`, `Cut`, `CutLeft`, `Truncate`, `TruncateLeft`, `StringWidth` |
| `ansi/width.go` | `src/width.rs` | `StringWidth`, `GraphemeWidth`/`WcWidth` methods, `FirstGraphemeCluster` |
...
| `ansi/sgr.go` | `src/sgr.rs` | SGR sequence parsing/encoding helpers |
| `ansi/mouse.go` | `src/mouse.rs` | Mouse SGR event parsing |
| `ansi/mode.go`, `mode_deprecated.go`, `modes.go` | `src/mode.rs` | Terminal mode constants and helpers |
| `ansi/cursor.go` | `src/cursor.rs` | Cursor movement/position sequences |
...
| `ansi/progress.go` | `src/progress.rs` | Terminal progress bar sequences |
| `ansi/progress_test.go` | `src/progress.rs` (tests) | Progress tests |
...
| `ansi/kitty.go` | `src/kitty.rs` | Kitty keyboard protocol |
| `ansi/kitty/decoder.go`, `ansi/kitty/encoder.go`, `ansi/kitty/graphics.go`, `ansi/kitty/options.go`, `ansi/kitty/writer.go` | `src/kitty.rs` | Kitty decoder/encoder/graphics |
...
| `ansi/background.go` | `src/background.rs` | Background color queries |
...
| `ansi/parser.go`, `ansi/parser_decode.go`, `ansi/parser_handler.go`, `ansi/parser_sync.go` | `src/parser.rs` | ANSI parser API |
...
| :--- | :--- | :--- |
| `ansi/style_test.go` | `tests/style_test.rs` | SGR style tests |
...
| `ansi/width_test.go` | `tests/width_test.rs` | Width tests |
| `ansi/hyperlink_test.go` | `tests/hyperlink_test.rs` | Hyperlink tests |
| `ansi/mouse_test.go` | `tests/mouse_test.rs` | Mouse parsing tests |
| `ansi/mode_test.go` | `tests/mode_test.rs` | Mode tests |
| `ansi/sgr_test.go` | `tests/sgr_test.rs` | SGR sequence tests |
...
skipped: `cellbuf`, `colors`, `conpty`, `editor`, `errors`, `examples/*`, `exp/*`, `input`,
`json`, `mosaic`, `pony/*`, `powernap/*`, `scripts`, `sshkey`, `term`, `termios`, `vcr`,
`vt`, `wcwidth`, `windows`, `xpty`, `fixtures`, plus the monorepo root files
...
| `clipperhouse/displaywidth` | v0.6.0 | `unicode-width` crate |
| `clipperhouse/uax29/v2` | v2.3.0 | `unicode-segmentation` crate |
| `clipperhouse/stringish` | v0.1.1 | `std` |
...
v0.11.6 for colorprofile v0.4.3, v0.11.7 for lipgloss/bubbletea/ultraviolet@20260703). Per
the multi-version rule, v0.11.2 is ported first; the other versions are produced by
diff-forwarding (`git diff ansi/v0.11.2..ansi/v0.11.6` etc.) and published as separate crate
versions. See `/Users/jonny/Projects/charming/DEPENDENCY_PLAN.md` §6.

## Porting Status

| Module group | Status |
| --- | --- |
| `ansi/style.go`, `color.go`, `util.go`, `width.go`, `wrap.go`, `truncate.go`, `hyperlink.go`, `method.go` | Ported & Tested |
| `ansi/mode.go`, `cursor.go`, `screen.go`, `title.go`, `reset.go`, `clipboard.go`, `mouse.go`, `sgr.go` | In progress |
| `parser*.go`, `kitty/*`, `sixel/*`, `iterm2/*`, remaining sequences | Pending (documented above) |
| Additional versions (v0.10.3, v0.11.6, v0.11.7) | Pending (diff-forward) |
